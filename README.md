...
> **画面プレビュー**：`pnpm dev --port 3001` を実行し、`http://localhost:3001/preview` を開くと、ログイン・バックエンドなしで主要画面を操作できます。データと会話はモックです。画面一覧、編集箇所、検証方法は [プレビューと編集ガイド](docs/prototype-screens.md) を参照してください。
...
舞台は、天の川をモチーフにした星川が流れる小さな街**「星川の街」**。背景は情報量を抑えたピクセルアート、NPCはレイヤー式のフラットイラストという意図的な異素材ミックスで、分身であるNPCを主役として際立たせます。橋を渡る = フォーマルなシーンへ行く、という世界の文法で、くらしエリアとしごとエリアを川が分けます。
