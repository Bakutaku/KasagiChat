...
- 768px未満では部屋、操作パネル、分身情報の順に縦スクロールで表示する。配置地点はタップまたは操作パネルの空き場所一覧から選べる。768〜1099pxでは左の分身パネルを省略し、部屋と操作パネルの幅を確保する。
...
室内の壁・窓・固定家具は [home-map-art.md](home-map-art.md) を参照。
外装を変更しても12個の配置地点とAPIスロットの対応は維持する。
