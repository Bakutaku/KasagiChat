...
`entities` は既存素材の `sprite` と位置・寸法を指定します。家は既存のoak／carpet／limestoneとbench／planter／lampだけを使っています。専用家具素材はありません。`spots[].entityId` は選択対象の置物を参照します。キャラクターや参加者は実行中に変わるため、JSONの `npcs`・`characters` は受け付けません。
